...
import { StatusBar, Keyboard, InAppBrowser, Splashscreen, LocalNotifications } from 'ionic-native';
...
export class MyApp {
    ...
    loadInfo(key){
        this._redminer.load('users/current.json', key).then(data => {
            let user = {name: '', src: '', email: ''};
            user.name = data.user.firstname+' '+data.user.lastname;
            user.src = this.get_gravatar(data.user.mail);
    ...
    get_gravatar(email) {
        ...
        return 'https://secure.gravatar.com/avatar/' + MD5(email) + '?rating=PG&size=150&default=retro';
    }
}